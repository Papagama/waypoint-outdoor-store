...
export function ProductCard({ product, priority = false }: { product: Product; priority?: boolean }) {
  ...
  return <article className="product-card">
    ...
    <div className="product-info"><div className="product-title"><div><p>{categoryName(product.category)}</p><Link href={`/product/${product.slug}`}><h3>{product.name}</h3></Link></div><button className={`heart-button ${isFavorite ? "is-active" : ""}`} aria-label={isFavorite ? "Удалить из избранного" : "Добавить в избранное"} onClick={() => toggleFavorite(product.id)}><Heart size={18} fill={isFavorite ? "currentColor" : "none"} /></button></div>
      <p className="product-description">{product.shortDescription}</p><div className="product-bottom"><span className="price">{formatPrice(product.price)} {product.previousPrice && <del>{formatPrice(product.previousPrice)}</del>}</span><button disabled={product.stock === "out"} className="quick-add" onClick={() => addProduct(product)}>{product.stock === "out" ? "Нет" : <><Plus size={15} /> В корзину</>}</button></div>
    </div>
  </article>;
}
