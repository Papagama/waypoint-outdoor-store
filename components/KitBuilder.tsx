...
const questions: { key: keyof KitAnswers; question: string; helper: string; options: string[] }[] = [
  ...
  { key: "budget", question: "Какой уровень комплекта?", helper: "Не про экономию на безопасности — про нужный объём решения.", options: ["Базовый", "Оптимальный", "Премиальный"] },
];
...
export function KitBuilder() {
  ...
  return <main className="kit-page"><section className="shell kit-hero"><p className="eyebrow">СБОРКА МАРШРУТА / 05</p><h1>Соберите<br /><i>комплект.</i></h1><p>Пять реальных параметров поездки. Один понятный стартовый комплект.</p></section>
    {!isResult ? <section className="shell builder-shell"><div className="builder-progress"><span>0{step + 1} / 0{questions.length}</span><div>{questions.map((_, index) => <i className={index <= step ? "complete" : ""} key={index} />)}</div><small>ВАШ МАРШРУТ</small></div><div className="question-panel"><div><p className="eyebrow">ПАРАМЕТР МАРШРУТА / 0{step + 1}</p><h2>{current.question}</h2><p>{current.helper}</p></div><div className="answers">{current.options.map((option, index) => <button className={answers[current.key] === option ? "selected" : ""} onClick={() => select(option)} key={option}><small>0{index + 1}</small><span>{option}</span>{answers[current.key] === option && <Check size={17} />}</button>)}</div><div className="builder-actions"><button className="button button-ghost" onClick={() => setStep((value) => Math.max(0, value - 1))} disabled={step === 0}><ArrowLeft size={16} /> Назад</button><button className="button button-dark" onClick={next} disabled={!answers[current.key]}>Дальше <ArrowRight size={16} /></button></div></div></section> : <section className="shell kit-result"><div className="kit-result-head"><div><p className="eyebrow">ВАШ МАРШРУТ / {answers.destination.toUpperCase()} · {answers.duration.toUpperCase()}</p><h2>{answers.duration === "Выходные" ? "ВЫХОДНЫЕ" : "ВАШ"} / <i>{answers.people === "2" ? "ДВОЕ" : answers.people}</i></h2><p>Стартовая конфигурация для выбранного маршрута. Это прозрачная демонстрационная рекомендация, не замена консультации или проверки совместимости.</p></div><button className="text-link" onClick={reset}><RotateCcw size={15} /> Начать заново</button></div><div className="kit-products">{recommended.map((product, index) => <article key={product.id}><span>0{index + 1}</span><img src={product.image} alt="" /><div><small>{categoryName(product.category).toUpperCase()}</small><h3>{product.name}</h3><p>{product.shortDescription}</p></div><b>{formatPrice(product.price)}</b></article>)}</div><div className="kit-total"><div><span>ПОЛНЫЙ КОМПЛЕКТ</span><b>{formatPrice(total)}</b><small>{recommended.length} предметов · доставка рассчитывается при оформлении</small></div><button className="button button-light" onClick={() => addBundle(recommended)}><ShoppingBag size={17} /> Добавить комплект в корзину</button></div><div className="kit-disclosure"><button onClick={() => setDetailsOpen(!detailsOpen)}>Почему такой комплект? <ChevronDown className={detailsOpen ? "rotated" : ""} size={17} /></button>{detailsOpen && <p>Логика набора меняет укрытие для группы и типа поездки, добавляет тепло для межсезонья и оставляет только базовые роли: переноска, сон, кухня и свет. До запуска продуктовая команда должна проверить алгоритм на реальных SKU.</p>}</div><Link href="/catalog" className="text-link">Посмотреть весь каталог <ArrowRight size={15} /></Link></section>}
  </main>;
}
